...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ========================
    // GPA Cache & Local Storage
    // ========================

    // Local Storage for Previous Calculations (CGPA Cache)
    const CACHE_VERSION = 'v1';
    const CACHE_ENABLED_KEY = 'gpa_cgpa_cache_enabled';
    const CACHE_DATA_KEY = 'gpa_cached_cgpa_data';
    const LAST_CALCULATION_KEY = 'gpa_last_calculation';
    ...
    // Save current CGPA calculation to cache
    const saveCgpatoCache = async () => {
        ...
        return new Promise(resolve => {
            const cacheData = {
                ...
                rating: ratingBadge.textContent,
                ratingClass: ratingBadge.className,
                totalCredits: metricCredits.textContent,
                cumulativePoints: metricPoints.textContent,
            ...
            localStorage.setItem(LAST_CALCULATION_KEY, Date.now().toString());
            resolve(cacheData);
        });
    };

    // Load previous CGPA calculation from cache
    const loadCgpafromCache = async () => {
        if (!isCacheEnabled()) {
        ...
        return new Promise(resolve => {
            ...
            if (!cachedData) {
                resolve({ error: 'No cached calculation found' });
                return;
            }
            ...
            try {
                ...
                resolve({ error: 'Failed to parse cached data' });
            }
        });
    };

    // Clear CGPA cache
    const clearCgpacache = async () => {
        return new Promise(resolve => {
    ...
    // Render cache tooltip with cached calculations
    const renderCacheTooltip = () => {
        const tooltip = document.getElementById('previous-calculations-tooltip');
        const content = document.getElementById('tooltip-content');

        if (!tooltip || !content) return;

        if (!isCacheEnabled()) {
            content.innerHTML = '<div class="tooltip-empty">Cache is disabled. Enable cache to store calculations.</div>';
            return;
        }

        loadCgpafromCache().then(data => {
            if (data.error) {
                content.innerHTML = `<div class="tooltip-empty">${data.error}</div>`;
                return;
            }
            ...
            const subjectPreview = data.subjects && data.subjects.length > 0 
                ? data.subjects.slice(0, 3).map(s => 
                    `<span class="calculation-subject-item" title="${s.name}">${s.name.split(' - ')[0]}</span>`
                ).join('') + (data.subjects.length > 3 ? ` <span>...and ${data.subjects.length - 3} more</span>` : '')
                : 'No subjects';

            content.innerHTML = `
                <div class="calculation-item" onclick="loadPreviousCgpaclicked()">
                    <div class="calculation-header">
                        <span class="calculation-semester">Semester ${data.semester}</span>
                        <span class="calculation-gpa">CGPA: ${data.gpa}</span>
                    </div>
                    <div class="calculation-meta">
                        <span title="Last calculated">📅 ${formattedDate}</span>
                        <span>📊 ${data.totalCredits || '0'} Credits</span>
                    </div>
                    <div class="calculation-subjects-preview" title="Subjects included">
                        ${subjectPreview}
                    </div>
                </div>
            `;
    ...
    // Handle loading previous CGPA from tooltip click
    const loadPreviousCgpaclicked = async () => {
        const tooltip = document.getElementById('previous-calculations-tooltip');
        ...
        try {
            const cachedData = await loadCgpafromCache();
            if (cachedData.error) {
                console.error('Failed to load cached data:', cachedData.error);
                return;
            }

            const semesterSelect = document.querySelector(`.sem-btn[data-sem="${cachedData.semester}"]`);
            if (!semesterSelect) {
                console.error(`Semester button for semester ${cachedData.semester} not found`);
                return;
            }
            ...
            setTimeout(() => {
                const updatedSubjects = cachedData.subjects.map(s => ({
                    ...s,
                    id: `${currentSemester}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
                }));
    ...
    // Initialize cache toggle button
    const initCacheToggle = () => {
        const cacheToggleBtn = document.getElementById('cache-toggle-btn');
        if (!cacheToggleBtn) {
            console.error('Cache toggle button not found');
            return;
        }

        const isEnabled = isCacheEnabled();
        if (isEnabled) {
            cacheToggleBtn.classList.add('active');
        }

        cacheToggleBtn.addEventListener('click', async () => {
            const isCurrentlyEnabled = cacheToggleBtn.classList.contains('active');

            if (isCurrentlyEnabled) {
                if (confirm('Disabling cache will remove all cached calculations. Continue?')) {
                    localStorage.removeItem(CACHE_ENABLED_KEY);
                    cacheToggleBtn.classList.remove('active');
                    await clearCgpacache();
                }
            } else {
                if (!confirm('Enable caching to store previous CGPA calculations for quick access. Continue?')) {
                    return;
                }
                localStorage.setItem(CACHE_ENABLED_KEY, 'true');
                cacheToggleBtn.classList.add('active');
                await saveCgpatoCache();
            }
    ...
    const initTooltipTracking = () => {
        const tooltip = document.getElementById('previous-calculations-tooltip');
        const cacheToggleBtn = document.getElementById('cache-toggle-btn');

        if (!tooltip || !cacheToggleBtn) return;

        let showTimeout;
        let hideTimeout;

        cacheToggleBtn.addEventListener('mouseenter', () => {
        ...
        cacheToggleBtn.addEventListener('mouseleave', () => {
        ...
        document.addEventListener('click', (e) => {
            if (!tooltip.contains(e.target) && !cacheToggleBtn.contains(e.target)) {
                tooltip.classList.remove('visible');
            }
        });
    };
    ...
    const initStorageListener = () => {
        window.addEventListener('storage', (e) => {
            if (e.key === CACHE_DATA_KEY || e.key === CACHE_ENABLED_KEY) {
                renderCacheTooltip();
            }
        });
    };
